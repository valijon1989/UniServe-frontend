...
function OrdersContent() {
  ...
  useEffect(() => {
    ...
    let alive = true;
    setLoading(true);
    getMyTaxiOrders(token)
      .then((data) => {
        if (alive) setOrders(data);
      })
      .catch(() => {
        if (alive) setError("Buyurtmalarni yuklab bo'lmadi.");
      })
      .finally(() => {
  ...
  const handleCancel = async (orderId: string) => {
    if (!token) return;
    try {
      const next = await updateTaxiOrderStatus(orderId, "cancelled", token);
      setOrders((prev) => prev.map((item) => (item.id === orderId ? { ...item, status: next.status } : item)));
    } catch {
      setError("Buyurtmani bekor qilishda xatolik.");
    }
  };
        ...
        {orders.map((order) => (
          <div key={order.id} className="card p-4 text-sm text-slate-200">
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-400">
              ...
              {order.listingId && (
                <Link href={`/taxi/${order.listingId}`} className="text-sky-300 hover:text-sky-200">
                  E'lonni ochish
                </Link>
              )}
            </div>
            {order.status === "pending" && (
              <button
                ...
                className="mt-3 rounded-lg border border-rose-500/40 bg-rose-500/10 px-3 py-1 text-xs text-rose-200 hover:bg-rose-500/20"
              >
                Bekor qilish
              </button>
            )}
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-10">
      <header className="mb-6">
        <h1 className="text-2xl font-semibold text-slate-100">Taxi buyurtmalarim</h1>
        <p className="mt-1 text-sm text-slate-400">
          Barcha taxi buyurtmalaringiz shu yerda.
        </p>
      </header>
      {content}
    </div>
